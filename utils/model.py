...
def generate_class_models(
    molecules_processed, variance_input, estimators_input, base_model_name
):
    ...
    for class_name, group in grouped:
        if len(group) < 20:  # Increased minimum sample size
            st.warning(
                f"Skipping class '{class_name}' due to insufficient data (only {len(group)} molecules)"
            )
            continue
        ...
        try:
            ...
            st.write(f"Test R² score: {model_results['test_r2']:.3f}")
            st.write(f"Mean CV R² score: {np.mean(model_results['cv_scores']):.3f}")
